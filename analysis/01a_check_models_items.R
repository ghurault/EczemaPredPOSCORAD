...
# - RW (for extent and subjective symptoms)
...
mdl_name <- match.arg(mdl_name, c("RW", "BinRW", "BinMC", "OrderedRW"))
severity_item <- match.arg(severity_item, c("extent", "intensity", "subjective", "B", "C"))
...
max_score <- case_when(severity_item == "extent" ~ 100,
                       severity_item == "subjective" ~ 10,
                       severity_item == "intensity" ~ 3,
                       severity_item == "B" ~ 18,
                       severity_item == "C" ~ 20)
reso <- case_when(severity_item %in% c("extent", "intensity", "B") ~ 1,
                  severity_item %in% c("subjective", "C") ~ 0.1)

file_dict <- get_results_files(outcome = severity_item, model = mdl_name)
...
if (FALSE) {
  ...
  apply(yrep1, 1, function(x) {(max(x) - min(x)) / max_score}) %>%
    ...
    scale_x_continuous(expand = c(0, 0), limits = c(0, 1)) +
    theme_bw(base_size = 15)
  # ggsave(paste0("results/prior_amplitude_", severity_item, "_", mdl_name, ".jpg"), width = 7, height = 8, units = "cm", dpi = 300, scale = 1.5)
...
lapply(sample(1:n_pt, min(n_pt, 4)),
       function(pid) {
         ggplot(data = fd %>% filter(Patient == pid) %>% drop_na(), aes(x = Time, y = Score)) +
           geom_line() +
           geom_point() +
           coord_cartesian(ylim = c(0, max_score)) +
           theme_bw(base_size = 15)
...
if (FALSE) {
  ...
  if (severity_item %in% c("intensity", "B")) {
