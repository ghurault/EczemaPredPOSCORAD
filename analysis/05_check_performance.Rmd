---
title: "Predictive performance of `r params$score` using the `r params$dataset` dataset"
...
if (score == "extent") {
  mdl_names <- c("uniform", "historical", "RW", "BinRW", "BinMC")
  comp_mdl <- "RW" # "BinRW"
} else if (score %in% c("itching", "sleep")) {
  mdl_names <- c("uniform", "historical", "RW", "BinRW")
  comp_mdl <- "RW" # "BinRW"
} else if (score %in% intensity_signs) {
  mdl_names <- c("uniform", "historical", "MC", "OrderedRW")
  comp_mdl <- "MC"
} else if (score %in% c("B")) {
  mdl_names <- c("uniform", "historical", "RW", "BinRW", "CombinedMC", "CombinedOrderedRW")
  comp_mdl <- "RW" # "CombinedMC"
} else if (score == "C") {
  mdl_names <- c("uniform", "historical", "RW", "BinRW", "CombinedBinRW")
  comp_mdl <- "RW" # "BinMRW"
} else if (score %in% c("SCORAD", "oSCORAD")) {
  mdl_names <- c("uniform", "historical", "RW", "AR1", "MixedAR1", "Smoothing", "EczemaPred")
  comp_mdl <- "RW"
}

res_files <- sapply(mdl_names,
                    function(m) {
                      here(get_results_files(outcome = score, model = m, dataset = dataset, val_horizon = params$t_horizon)$Val)
                    })
...
pl <- lapply(metrics,
             function(metric) {
               ...
               p1 <- perf %>%
                 ...
                 plot_learning_curves(perf = ., metric = ifelse(score %in% c("SCORAD", "oSCORAD"), "", metric), fc_it = fc_it)
